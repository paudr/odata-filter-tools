...
function dateReader (string, index) {
  const reader = getRegexReader(/^(\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+(?:[+-][0-2]\d:[0-5]\d|Z))/)
...
const tokenReaders = [{
  ...
  name: 'DATE',
  test: (string, index) => /^\d{4}-[01]\d-[0-3]\dT[0-2]\d:[0-5]\d:[0-5]\d\.\d+(?:[+-][0-2]\d:[0-5]\d|Z)/.test(string.substring(index)),
  reader: dateReader
}, {
  name: 'NUMBER',
